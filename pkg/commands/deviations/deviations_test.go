...
func TestRun_ByDeviationNameReturnsSelectedDeviations(t *testing.T) {
	...
	selected, err := Run(context.Background(), cl, NewDeviationOptions("default", WithDeviationName("dev-1")))
	if err != nil {
		t.Fatalf("Run() unexpected error: %v", err)
	}
...
func TestRun_NoSelectionReturnsNil(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
...
func TestRun_RevertClearsSelectedDeviations(t *testing.T) {
	...
	selected, err := Run(context.Background(), cl, NewDeviationOptions("default", WithTarget("target-1"), WithRevert(true)))
	if err != nil {
		t.Fatalf("Run() unexpected error: %v", err)
	}
	if selected != nil {
...
func TestRun_NoTargetOrDeviationReturnsError(t *testing.T) {
	...
	if err == nil || err.Error() != "deviation or target not set" {
		t.Fatalf("Run() error = %v, want %q", err, "deviation or target not set")
	}
	if selected != nil {
		t.Fatalf("selected = %v, want nil", selected)
	}
...
func TestRun_NoDeviationsFoundReturnsError(t *testing.T) {
	...
	selected, err := Run(context.Background(), cl, NewDeviationOptions("default", WithTarget("target-1")))
	if err == nil || err.Error() != "no deviations found" {
		t.Fatalf("Run() error = %v, want %q", err, "no deviations found")
	}
	if selected != nil {
		t.Fatalf("selected = %v, want nil", selected)
	}
